...
//  spot-the-scientist
//
//  Created by Daniel Mansour on 12/29/21.
//
...
class ViewController: UIViewController, ARSCNViewDelegate {
    ...
    func renderer(_ renderer: SCNSceneRenderer, nodeFor anchor: ARAnchor) -> SCNNode? {
        ...
        let bioNode = textNode(scientist.bio, font: UIFont.systemFont(ofSize: 4.0), maxWidth: 100)
...
extension SCNNode {
    ...
    func pivotOnTopLeft() {
        ...
        pivot = SCNMatrix4MakeTranslation(min.x, (max.y - min.y) + min.y, 0.0)
    }

    func pivotOnTopCenter() {
        let (min, max) = boundingBox
        pivot = SCNMatrix4MakeTranslation((max.x - min.x) / 2 + min.x, (max.y - min.y) + min.y, 0.0)
    }
}
